...
parser.add_argument("dateipfad", help="Pfad zur Textdatei, die den zu sprechenden Text enthält")
...
parser.add_argument("--dateiname", help="Optionaler Dateiname für die gespeicherte WAV-Datei. Standard wird der Textname verwendet.")
args = parser.parse_args()

# 📁 Ziel-Datei vorbereiten
output_dir = "/opt/sound/hilfe"
os.makedirs(output_dir, exist_ok=True)

# Lese den Text aus der Datei
try:
    with open(args.dateipfad, 'r', encoding='utf-8') as file:
        text = file.read().strip()  # Text aus der Datei lesen und überschüssige Leerzeichen entfernen
except Exception as e:
    print(f"❌ Fehler beim Lesen der Datei: {e}")
    sys.exit(1)

# Verwende den angegebenen Dateinamen oder erstelle ihn basierend auf dem Text
if args.dateiname:
    filename = args.dateiname.lower().replace(" ", "_") + ".wav"
else:
    filename = text.lower().replace(" ", "_") + ".wav"
...
# 🎤 Modell laden
tts = TTS("tts_models/de/thorsten/vits", progress_bar=False, gpu=True)
